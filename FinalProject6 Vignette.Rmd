...
`graphci` runs the `bootstrapping` function and returns its same outputs, so the output of `bootstrapping(x = c("age", "workclass", "hours.per.week"), y = "sex", data = adult, alpha = 0.05, B = 8)` is stored in `boot2` (recalling the default value of $\alpha$ is 0.05).
